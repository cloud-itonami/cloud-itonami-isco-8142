(ns plasticcoord.actor-test
  (:require [clojure.test :refer [deftest is testing]]
            [plasticcoord.actor :as actor]
...
(deftest commits-a-registered-work-log
  (let [st (fresh-store)
        ...
        request {:molder-id "molder-1" :op :log-work-record :stake :low
                  :facility-id "F-1" :task "production run progress log"}
        result (actor/run-request! graph request {} "thread-1")]
    (is (= :done (:status result)))
    (is (some? (get-in result [:state :record])))
    (is (= 1 (count (store/records-of st "molder-1"))))))
...
(deftest holds-an-unregistered-facility-proposal
  (let [st (fresh-store)
    ...
    (is (empty? (store/records-of st "molder-1")))))

(deftest interrupts-then-approves-safety-concern-on-human-approval
  (let [st (fresh-store)
        graph (actor/build-graph {:store st})
        request {:molder-id "molder-1" :op :flag-safety-concern :stake :low
                  :facility-id "F-1" :hazard-type :machinery-hazard}
...
(deftest holds-a-scope-excluded-op-even-at-high-confidence
  (testing "an actor run can never commit a proposal that would finalize a molding-operation-execution decision, regardless of disposition path"
    (let [st (fresh-store)
          graph (actor/build-graph {:store st})
          request {:molder-id "molder-1" :op :finalize-molding-decision :stake :low
                    :facility-id "F-1" :task "molding decision"}
...
(deftest holds-a-plant-safety-clearance-op-even-at-high-confidence
  (testing "an actor run can never commit a proposal that would finalize a plant-safety-clearance decision, regardless of disposition path"
    (let [st (fresh-store)
          graph (actor/build-graph {:store st})
          request {:molder-id "molder-1" :op :declare-plant-safety-cleared :stake :low
...
(deftest holds-an-override-plant-safety-officer-op-even-at-high-confidence
  (testing "an actor run can never commit a proposal that would override a plant safety officer's judgment, regardless of disposition path"
    (let [st (fresh-store)
          graph (actor/build-graph {:store st})
          request {:molder-id "molder-1" :op :override-plant-safety-officer-judgment :stake :low
...
(deftest holds-an-extrusion-finalize-op-even-at-high-confidence
  (testing "an actor run can never commit a proposal that would finalize an extrusion-operation-execution decision, regardless of disposition path"
    (let [st (fresh-store)
          graph (actor/build-graph {:store st})
          request {:molder-id "molder-1" :op :finalize-extrusion-operation :stake :low
                    :facility-id "F-1" :task "extrusion decision"}
