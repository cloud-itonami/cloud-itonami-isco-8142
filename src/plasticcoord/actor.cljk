(ns plasticcoord.actor
  "PlasticCoordActor — the ISCO-08 8142 plastic products machine
  operators plant scheduling/logistics coordination actor as a
  ...
  The unconditional invariant: the Plastic Plant Scheduling
  Coordination Advisor can never directly commit an operating record
  the PlasticCoordGovernor refuses, and can never make this actor
  finalize a molding-operation-execution decision (e.g. deciding to
  proceed with a specific injection-molding or extrusion run) or a
  plant-safety-clearance decision (e.g. declaring a plant or a molded
  batch safe for handling), and can never override a plant safety
  ...
  SCHEDULING/LOGISTICS ONLY — it never operates injection-molding or
  extrusion equipment itself, and it never makes a
...
(defn build-graph
  "Build a compiled PlasticCoordActor graph. `store` implements
  `plasticcoord.store/Store`. `advisor` implements
  `plasticcoord.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:molder-id (:molder-id request)
                                    :op (:op proposal)
